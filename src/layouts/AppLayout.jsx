...
export default function AppLayout({ auth }) {
  ...
  const actions = useMemo(() => ({
    ...
    newLead: (type) => {
      if (type) setLeadModal({ open: true, mode: "create", lead: { type } });
  ...
  return (
    <div className="min-h-screen bg-[#f7f5fc] text-[#130d2e]">
      ...
      <div className={`${collapsed ? "lg:pl-20" : "lg:pl-72"} transition-all`}>
        <Topbar auth={auth} pageTitle={titles[page]} onNewLead={() => actions.newLead(pageLeadTypes[page])} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} globalSearch={globalSearch} setGlobalSearch={setGlobalSearch} role={role} setRole={setRole} resetDemo={crm.resetDemo} />
        <main className="p-4 lg:p-6">{pages[page]}</main>
      </div>
      <LeadTypeChooser open={leadTypeChooserOpen} onClose={() => setLeadTypeChooserOpen(false)} onSelect={openLeadForm} />
