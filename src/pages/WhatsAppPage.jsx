  ...
  return (
    <div className="grid gap-6">
      <div className="dialyatech-panel p-5">
        <span className="dialyatech-chip">Interakt automation</span>
      ...
      <section className="grid gap-4 lg:grid-cols-5">
        {[
          ...
          ["D2C Customer", "Send D2C refill or cart recovery"],
          ["Follow-up", "Send due sales reminder"],
          ["Pharmacy", "Send onboarding pitch"],
          ["Delivery Partner", "Send rider onboarding pitch"],
        ].map(([label, note]) => (
          <button key={label} className="crm-card p-5 text-left hover:border-[#d8c9ff] hover:bg-[#f7f5fc]" onClick={() => actions.sendWhatsApp(leads.find((lead) => label === "Follow-up" || lead.type === label) || leads[0])}>
        ...
        <div className="crm-card overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[820px] w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-[#ede6ff]">{messages.map((msg) => <tr key={msg.id}><td className="px-4 py-3 font-bold">{msg.leadName}</td><td className="px-4 py-3">{msg.template}</td><td className="px-4 py-3"><StatusBadge value={msg.status} /></td><td className="px-4 py-3">{msg.sentBy}</td><td className="px-4 py-3">{msg.sentAt}</td></tr>)}</tbody>
            </table>
          </div>
        </div>
